...
export default function RoleTracker() {
  ...
  function addTodo(appId) {
    ...
    setApps(apps.map(a => a.id === appId ? { ...a, todos: [...(a.todos || []), todo] } : a))
  }
  function updateTodo(appId, tId, field, value) {
    setApps(apps.map(a => a.id !== appId ? a : {
      ...a, todos: (a.todos || []).map(t => t.id === tId ? { ...t, [field]: value } : t)
    }))
  }
  function removeTodo(appId, tId) {
    setApps(apps.map(a => a.id !== appId ? a : {
      ...a, todos: (a.todos || []).filter(t => t.id !== tId)
    }))
  }
  ...
  // Shared expanded form used in both card and table views
  function ExpandedForm({ app }) {
            ...
            <div className="space-y-2">
              {[...(app.outreachLog || [])].sort((a, b) => b.date.localeCompare(a.date)).map(o => (
                <div key={o.id} className="grid grid-cols-[auto_auto_1fr_auto] gap-2 items-center bg-[#F8F5F2] rounded-lg px-3 py-2">
                  ...
                  />
                  <button onClick={() => removeOutreach(app.id, o.id)} className="text-[#D8E4EC] hover:text-[#FF5E5B] transition-colors cursor-pointer flex-shrink-0">
                    <Trash2 size={13} />
                  </button>
                </div>
              ))}
          ...
          {(app.todos || []).map(t => (
            <div key={t.id} className="flex items-center gap-2 py-1.5">
              <input
                type="checkbox"
                checked={t.done}
                onChange={e => updateTodo(app.id, t.id, 'done', e.target.checked)}
                className="accent-[#263746]"
              />
              <input
              ...
              />
              <button onClick={() => removeTodo(app.id, t.id)} className="text-[#D8E4EC] hover:text-[#FF5E5B] cursor-pointer transition-colors">
                <Trash2 size={13} />
              </button>
            </div>
          ))}
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {['Tailor resume', 'Write cover letter', 'Research company', 'Connect on LinkedIn', 'Send thank-you email', 'Prepare for interview'].map(t => (
                <button
                  key={t}
                  onClick={() => {
                    const todo = { id: Date.now(), text: t, done: false }
                    setApps(apps.map(a => a.id === app.id ? { ...a, todos: [...(a.todos || []), todo] } : a))
                  }}
                  className="text-xs px-2.5 py-1 rounded-full bg-[#EEF3FA] text-[#4A5C6B] hover:bg-[#263746] hover:text-white cursor-pointer transition-colors"
                >
                  + {t}
                </button>
              ))}
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center gap-1.5 text-sm font-medium px-3 py-2 rounded-lg cursor-pointer transition-colors ${viewMode === 'cards' ? 'bg-[#263746] text-white' : 'bg-white border border-[#D8E4EC] text-[#4A5C6B] hover:bg-[#F5F9FD]'}`}
            title="Card view"
          >
            <LayoutGrid size={15} />
          </button>
          <button
            ...
            className={`flex items-center gap-1.5 text-sm font-medium px-3 py-2 rounded-lg cursor-pointer transition-colors ${viewMode === 'table' ? 'bg-[#263746] text-white' : 'bg-white border border-[#D8E4EC] text-[#4A5C6B] hover:bg-[#F5F9FD]'}`}
            title="Table view"
          >
            <List size={15} />
          </button>
          ...
          {apps.map((app) => (
            <div key={app.id} className={`bg-white rounded-xl overflow-hidden border-2 ${app.vibe === 'green' ? 'border-emerald-400' : app.vibe === 'amber' ? 'border-amber-400' : app.vibe === 'red' ? 'border-red-400' : 'border-[#D8E4EC]'}`}>
              ...
              >
                ...
                {app.workType && (
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full flex-shrink-0 ${WORK_TYPE_COLORS[app.workType] || ''}`}>
                    {app.workType}
                  </span>
                )}
              ...
              {apps.map((app) => {
                ...
                return (
                  <>
                    <tr
                      key={app.id}
                      className={`border-b border-[#EEF3FA] cursor-pointer hover:bg-[#F5F9FD] transition-colors ${isExpanded ? 'bg-[#F8F5F2]' : ''}`}
                      onClick={() => setExpanded(isExpanded ? null : app.id)}
                    >
                      <td className="px-4 py-3">
                        <span className={`text-xs font-medium px-2 py-0.5 rounded-full whitespace-nowrap ${STATUS_COLORS[app.status] || 'bg-[#F5F9FD] text-[#7A8FA3]'}`}>
                          {app.status}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 text-[#263746] max-w-[140px]">
                        <div className="flex items-center gap-1 truncate">
                          ...
                          {app.jobUrl && (
                            <a href={app.jobUrl} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} className="text-[#6D99F2] hover:text-[#263746] flex-shrink-0">
                      ...
                      <td className="px-4 py-3">
                        {app.workType ? (
                          <span className={`text-xs font-medium px-2 py-0.5 rounded-full whitespace-nowrap ${WORK_TYPE_COLORS[app.workType] || ''}`}>
                            {app.workType}
                          </span>
                        ) : <span className="text-[#7A8FA3]">—</span>}
                      </td>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        {app.followUpDate ? (
                          <span className={followUpOverdue ? 'text-red-500 font-semibold' : 'text-[#7A8FA3]'}>
                            {app.followUpDate}
                          </span>
                        ) : <span className="text-[#7A8FA3]">—</span>}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                          <button
                            onClick={() => setExpanded(isExpanded ? null : app.id)}
                            className="text-[#7A8FA3] hover:text-[#263746] cursor-pointer transition-colors"
                            title="Expand"
                          >
