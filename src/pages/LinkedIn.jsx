...
export default function LinkedIn() {
  ...
  function addOutreach() { const o = newOutreach(); setOutreach([...outreach, o]); setExpanded(o.id) }
  ...
  return (
    <div className="w-full">
      <div className="mb-6">
        ...
        <p className="text-sm text-[#7A8FA3]">Profile checklist, outreach log and activity tracker.</p>
      </div>
      ...
      <div className="mt-8">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-[#7A8FA3] mt-0.5">Track every connection request, message, and follow-up.</p>
          </div>
        ...
        {outreach.length === 0 && (
          <div className="bg-white rounded-xl border border-[#D8E4EC] p-10 text-center mb-6">
            ...
            <p className="text-[#7A8FA3] text-sm max-w-md mx-auto leading-relaxed mb-5">Log each connection request, follow-up, and conversation. Keeps you accountable and makes sure no one slips through the cracks.</p>
        ...
        <div className="space-y-3 mb-4">
          {outreach.map(o => (
            <div key={o.id} className="bg-white rounded-xl border border-[#D8E4EC] overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-[#F5F9FD] transition-colors" onClick={() => toggle(o.id)}>
                ...
                <div className="flex items-center gap-3 flex-shrink-0 ml-3">
                  {o.dateConnected && <span className="hidden md:block text-xs text-[#7A8FA3]">{new Date(o.dateConnected).toLocaleDateString('en-AU', { day: 'numeric', month: 'short' })}</span>}
                  <button onClick={e => { e.stopPropagation(); removeOutreach(o.id) }} className="text-[#D8E4EC] hover:text-[#FF5E5B] transition-colors cursor-pointer"><Trash2 size={15} /></button>
              ...
              {expanded === o.id && (
                <div className="px-5 pb-5 border-t border-[#EEF3FA]">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                    <div><label className={labelCls}>Person</label><input className={inputCls} value={o.person} onChange={e => updOutreach(o.id,'person',e.target.value)} placeholder="Full name" /></div>
                    <div><label className={labelCls}>Company</label><input className={inputCls} value={o.company} onChange={e => updOutreach(o.id,'company',e.target.value)} placeholder="Company" /></div>
                    <div><label className={labelCls}>Their Role</label><input className={inputCls} value={o.role} onChange={e => updOutreach(o.id,'role',e.target.value)} placeholder="Job title" /></div>
                    <div><label className={labelCls}>Date Connected</label><input className={inputCls} type="date" value={o.dateConnected} onChange={e => updOutreach(o.id,'dateConnected',e.target.value)} /></div>
                    <div>
                    ...
                    <div><label className={labelCls}>Follow-up Date</label><input className={inputCls} type="date" value={o.followUp} onChange={e => updOutreach(o.id,'followUp',e.target.value)} /></div>
                    <div className="sm:col-span-2 lg:col-span-3"><label className={labelCls}>Message Sent</label><input className={inputCls} value={o.message} onChange={e => updOutreach(o.id,'message',e.target.value)} placeholder="Summary of your message" /></div>
                    <div className="sm:col-span-2 lg:col-span-3"><label className={labelCls}>Notes</label><input className={inputCls} value={o.notes} onChange={e => updOutreach(o.id,'notes',e.target.value)} placeholder="Any other notes" /></div>
                  </div>
                </div>
              )}
            </div>
        ...
        {outreach.length > 0 && (
          <div className="flex justify-between items-center text-xs text-[#7A8FA3] mb-6">
